...
function printPDS(id) {
  const e = employees.find(x => x.id === id); if (!e) { toast('Employee not found.', 'error'); return; }
  const tr = empTr(id); const pr = e.personal; const q = e.questions;
  const refs = e.references || [{name:'',address:'',contact:''},{name:'',address:'',contact:''},{name:'',address:'',contact:''}];
  const v = s => s || '';
  const yn = val => val ? `<b>☑ Yes</b> &nbsp; ☐ No` : `☐ Yes &nbsp; <b>☑ No</b>`;

  const eduLevels = ['Elementary','Secondary','Vocational / Trade Course','College','Graduate Studies'];
  const eduRows = eduLevels.map(level => {
    const f = e.education.find(x => x.level.toLowerCase().startsWith(level.split(' ')[0].toLowerCase()));
    return `<tr><td style="font-size:7.5px;font-weight:bold;padding:2px 4px;background:#f0f0f0">${level}</td><td style="padding:2px 4px">${f?v(f.school):''}</td><td style="padding:2px 4px">${f?v(f.course):''}</td><td style="text-align:center;padding:2px 3px">${f?v(f.from):''}</td><td style="text-align:center;padding:2px 3px">${f?v(f.to):''}</td><td style="text-align:center;padding:2px 3px">${f?v(f.units):''}</td><td style="text-align:center;padding:2px 3px">${f?v(f.yearGrad):''}</td><td style="padding:2px 4px">${f?v(f.honors):''}</td></tr>`;
  }).join('');

  let eligRows = e.eligibility.map(r => `<tr><td style="padding:2px 4px">${v(r.name)}</td><td style="text-align:center;padding:2px 3px">${v(r.rating)}</td><td style="text-align:center;padding:2px 3px">${v(r.dateConf)}</td><td style="padding:2px 4px">${v(r.place)}</td><td style="text-align:center;padding:2px 3px">${v(r.licNo)}</td><td style="text-align:center;padding:2px 3px">${v(r.licValid)}</td></tr>`).join('');
  for (let i = e.eligibility.length; i < 5; i++) eligRows += `<tr><td style="padding:3px 4px">&nbsp;</td><td></td><td></td><td></td><td></td><td></td></tr>`;

  let weRows = e.workExp.map(r => `<tr><td style="text-align:center;padding:2px 3px">${v(r.from)}</td><td style="text-align:center;padding:2px 3px">${v(r.to)}</td><td style="padding:2px 4px">${v(r.position)}</td><td style="padding:2px 4px">${v(r.dept)}</td><td style="text-align:center;padding:2px 3px">${v(r.salary||'')}</td><td style="text-align:center;padding:2px 3px">${v(r.status)}</td><td style="text-align:center;padding:2px 3px">${v(r.govtService)}</td></tr>`).join('');
  for (let i = e.workExp.length; i < 8; i++) weRows += `<tr><td style="padding:4px 4px">&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`;

  let volRows = e.voluntaryWork.map(r => `<tr><td style="padding:2px 4px">${v(r.org||'')}</td><td style="text-align:center;padding:2px 3px">${v(r.from||'')}</td><td style="text-align:center;padding:2px 3px">${v(r.to||'')}</td><td style="text-align:center;padding:2px 3px">${v(r.hours||'')}</td><td style="padding:2px 4px">${v(r.position||'')}</td></tr>`).join('');
  for (let i = e.voluntaryWork.length; i < 5; i++) volRows += `<tr><td style="padding:4px 4px">&nbsp;</td><td></td><td></td><td></td><td></td></tr>`;

  let trRows = tr.map(t => `<tr><td style="padding:2px 4px">${v(t.title)}</td><td style="text-align:center;padding:2px 3px">${v(t.from)}</td><td style="text-align:center;padding:2px 3px">${v(t.to)}</td><td style="text-align:center;padding:2px 3px">${v(t.hours)}</td><td style="text-align:center;padding:2px 3px">${v(t.type)}</td><td style="padding:2px 4px">${v(t.conductedBy)}</td></tr>`).join('');
  for (let i = tr.length; i < 6; i++) trRows += `<tr><td style="padding:4px 4px">&nbsp;</td><td></td><td></td><td></td><td></td><td></td></tr>`;

  let childRows = e.family.children.map(c => `<tr><td style="padding:2px 4px">${v(c.name)}</td><td style="text-align:center;padding:2px 3px">${v(c.dob)}</td></tr>`).join('');
  for (let i = e.family.children.length; i < 8; i++) childRows += `<tr><td style="padding:3px 4px">&nbsp;</td><td></td></tr>`;

  const w = window.open('','_blank');
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>CS Form 212 — ${pr.surname}, ${pr.firstName}</title>
<style>
...
h1{font-size:11px;font-weight:bold;text-align:center;letter-spacing:.5px;margin-bottom:1px}
.warn{font-size:6.5px;color:#c00;text-align:center;margin-bottom:1px;line-height:1.4}
.guide{font-size:6.5px;text-align:center;margin-bottom:3px;line-height:1.4}
.shdr{background:#000;color:#fff;font-size:7.5px;font-weight:bold;text-transform:uppercase;padding:2px 5px;letter-spacing:.3px;margin-top:2px}
table{width:100%;border-collapse:collapse}
td,th{border:.4px solid #888;vertical-align:top;font-size:7.5px}
th{background:#d0d0d0;font-weight:bold;font-size:7px;text-align:center;padding:2px 3px}
.lbl{font-size:7px;color:#333;display:block;text-transform:uppercase;line-height:1.3}
.val{font-size:8.5px;display:block;min-height:10px}
.nb td{border:none}
.sig-area{display:flex;gap:20px;margin-top:4px;padding-top:3px;border-top:.4px solid #888}
.sig-box{flex:1;font-size:7px}
.sig-line{border-bottom:.5px solid #000;height:18px;margin-bottom:2px}
.pfoot{font-size:7px;text-align:center;margin-top:3px;border-top:.4px solid #888;padding-top:2px}
<\/style><\/head><body>
...
<div class="guide">READ THE ATTACHED GUIDE TO FILLING OUT THE PERSONAL DATA SHEET (PDS) BEFORE ACCOMPLISHING THE PDS FORM.<br>Print legibly if accomplished through own handwriting. Tick appropriate boxes (✓) and use separate sheet if necessary. Indicate N/A if not applicable. DO NOT ABBREVIATE.</div>

<div class="shdr">I. PERSONAL INFORMATION</div>
<table>
<tr>
  <td colspan="3" style="padding:2px 4px">
    <span class="lbl">1. SURNAME</span><span class="val" style="font-size:10px;font-weight:bold;text-transform:uppercase">${v(pr.surname)}</span>
  </td>
  <td rowspan="3" style="padding:2px 4px" width="23%">
    <span class="lbl">16. CITIZENSHIP</span><br>
    <span style="font-size:8px">${v(pr.citizenship)}</span><br><br>
    <span style="font-size:7px">If holder of dual citizenship, please indicate the details.<br>
    ${pr.dualCitizenship?'☑':'☐'} By birth &nbsp; ${pr.dualCitizenship?'☑':'☐'} By naturalization</span><br>
    <span style="font-size:7px">Country: ${v(pr.dualCountry)}</span>
  </td>
</tr>
<tr>
  <td colspan="2" style="padding:2px 4px">
    <span class="lbl">2. FIRST NAME</span><span class="val" style="font-size:10px;font-weight:bold;text-transform:uppercase">${v(pr.firstName)}</span>
  </td>
  <td style="padding:2px 4px">
    <span class="lbl">NAME EXTENSION (JR., SR)</span><span class="val">${v(pr.nameExt)}</span>
  </td>
</tr>
<tr>
  <td colspan="3" style="padding:2px 4px">
    <span class="lbl">MIDDLE NAME</span><span class="val" style="font-size:10px;font-weight:bold;text-transform:uppercase">${v(pr.middleName)}</span>
  </td>
</tr>
<tr>
  <td style="padding:2px 4px" width="18%">
    <span class="lbl">3. DATE OF BIRTH (dd/mm/yyyy)</span><span class="val">${v(pr.dob)}</span>
  </td>
  <td colspan="2" style="padding:2px 4px" width="40%">
    <span class="lbl">17. RESIDENTIAL ADDRESS</span>
    <table class="nb" style="width:100%">
      <tr><td style="width:50%;padding:0 2px 0 0"><span class="lbl">House/Block/Lot No.</span><span class="val">${v(pr.residHouseNo)}</span></td><td style="padding:0"><span class="lbl">Street</span><span class="val">${v(pr.residStreet)}</span></td></tr>
      <tr><td style="padding:0 2px 0 0"><span class="lbl">Subdivision/Village</span><span class="val">${v(pr.residSubdiv)}</span></td><td style="padding:0"><span class="lbl">Barangay</span><span class="val">${v(pr.residBrgy)}</span></td></tr>
      <tr><td style="padding:0 2px 0 0"><span class="lbl">City/Municipality</span><span class="val">${v(pr.residCity)}</span></td><td style="padding:0"><span class="lbl">Province</span><span class="val">${v(pr.residProv)}</span></td></tr>
      <tr><td colspan="2" style="padding:0"><span class="lbl">ZIP CODE</span><span class="val">${v(pr.residZip)}</span></td></tr>
    </table>
  </td>
  <td style="padding:2px 4px" width="19%">
    <span class="lbl">6. CIVIL STATUS</span><br>
    <span style="font-size:7px">${['Single','Married','Widow/er','Separated','Solo Parent','Others'].map(s=>`${pr.civil===s?'☑':'☐'} ${s}`).join('<br>')}</span>
  </td>
</tr>
<tr>
  <td style="padding:2px 4px">
  ...
  <td colspan="2" rowspan="2" style="padding:2px 4px">
    <span class="lbl">18. PERMANENT ADDRESS</span>
    <table class="nb" style="width:100%">
      <tr><td style="width:50%;padding:0 2px 0 0"><span class="lbl">House/Block/Lot No.</span><span class="val">${v(pr.permHouseNo)}</span></td><td style="padding:0"><span class="lbl">Street</span><span class="val">${v(pr.permStreet)}</span></td></tr>
      <tr><td style="padding:0 2px 0 0"><span class="lbl">Subdivision/Village</span><span class="val">${v(pr.permSubdiv)}</span></td><td style="padding:0"><span class="lbl">Barangay</span><span class="val">${v(pr.permBrgy)}</span></td></tr>
      <tr><td style="padding:0 2px 0 0"><span class="lbl">City/Municipality</span><span class="val">${v(pr.permCity)}</span></td><td style="padding:0"><span class="lbl">Province</span><span class="val">${v(pr.permProv)}</span></td></tr>
      <tr><td colspan="2" style="padding:0"><span class="lbl">ZIP CODE</span><span class="val">${v(pr.permZip)}</span></td></tr>
    </table>
  </td>
  <td style="padding:2px 4px">
    <span class="lbl">5. SEX AT BIRTH</span><br>
    <span style="font-size:8px">${pr.sex==='Male'?'☑':'☐'} Male &nbsp; ${pr.sex==='Female'?'☑':'☐'} Female</span>
  </td>
</tr>
<tr>
  <td style="padding:2px 4px"><span class="lbl">7. HEIGHT (m)</span><span class="val">${v(pr.height)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">19. TELEPHONE NO.</span><span class="val">${v(pr.telNo)}</span></td>
</tr>
<tr>
  <td style="padding:2px 4px"><span class="lbl">8. WEIGHT (kg)</span><span class="val">${v(pr.weight)}</span></td>
  <td colspan="2" style="padding:2px 4px"><span class="lbl">20. MOBILE NO.</span><span class="val">${v(pr.mobileNo)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">9. BLOOD TYPE</span><span class="val">${v(pr.blood)}</span></td>
</tr>
<tr>
  <td style="padding:2px 4px"><span class="lbl">10. UMID ID NO.</span><span class="val">${v(pr.umid)}</span></td>
  <td colspan="2" style="padding:2px 4px"><span class="lbl">21. E-MAIL ADDRESS (if any)</span><span class="val">${v(pr.email)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">11. PAG-IBIG ID NO.</span><span class="val">${v(pr.pagibig)}</span></td>
</tr>
<tr>
  <td style="padding:2px 4px"><span class="lbl">12. PHILHEALTH NO.</span><span class="val">${v(pr.philhealth)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">13. PHILSYS NUMBER (PSN)</span><span class="val">${v(pr.philsys)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">14. TIN NO.</span><span class="val">${v(pr.tin)}</span></td>
  <td style="padding:2px 4px"><span class="lbl">15. AGENCY EMPLOYEE NO.</span><span class="val">${v(pr.agencyNo)}</span></td>
</tr>
</table>
...
<table>
<tr>
  <td width="42%" style="padding:2px 4px"><span class="lbl">22. SPOUSE'S SURNAME</span><span class="val">${v(e.family.spouseSurname)}</span></td>
  <td rowspan="7" width="58%">
    <table style="width:100%;height:100%;border-collapse:collapse">
      <tr><th colspan="2" style="background:#d0d0d0;padding:2px 3px">23. NAME of CHILDREN (Write full name and list all)</th><th style="background:#d0d0d0;padding:2px 3px">DATE OF BIRTH (dd/mm/yyyy)</th></tr>
      ${childRows}
    </table>
  </td>
</tr>
<tr><td style="padding:2px 4px"><span class="lbl">FIRST NAME</span><span class="val">${v(e.family.spouseFirstName)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">MIDDLE NAME</span><span class="val">${v(e.family.spouseMiddleName)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">OCCUPATION</span><span class="val">${v(e.family.spouseOccupation)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">EMPLOYER/BUSINESS NAME</span><span class="val">${v(e.family.spouseEmployer)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">BUSINESS ADDRESS</span><span class="val">${v(e.family.spouseBusiness)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">TELEPHONE NO.</span><span class="val">${v(e.family.spouseTel)}</span></td></tr>
<tr>
  <td colspan="2" style="padding:2px 4px">
    <span class="lbl">24. FATHER'S SURNAME &nbsp;&nbsp;&nbsp; FIRST NAME &nbsp;&nbsp;&nbsp; MIDDLE NAME &nbsp;&nbsp;&nbsp; EXTENSION (JR., SR)</span>
    <span class="val">${v(e.family.fatherSurname)} &nbsp;&nbsp;&nbsp; ${v(e.family.fatherFirstName)} &nbsp;&nbsp;&nbsp; ${v(e.family.fatherMiddleName)} &nbsp;&nbsp;&nbsp; ${v(e.family.fatherExt)}</span>
  </td>
</tr>
<tr>
  <td colspan="2" style="padding:2px 4px">
    <span class="lbl">25. MOTHER'S MAIDEN NAME &nbsp;&nbsp;&nbsp; SURNAME &nbsp;&nbsp;&nbsp; FIRST NAME &nbsp;&nbsp;&nbsp; MIDDLE NAME</span>
    <span class="val">${v(e.family.motherSurname)} &nbsp;&nbsp;&nbsp; ${v(e.family.motherFirstName)} &nbsp;&nbsp;&nbsp; ${v(e.family.motherMiddleName)}</span>
  </td>
</tr>
</table>
...
<table>
<tr>
  <th rowspan="2" width="14%">26. LEVEL</th>
  <th rowspan="2" width="25%">NAME OF SCHOOL<br>(Write in full)</th>
  <th rowspan="2" width="22%">BASIC EDUCATION/DEGREE/COURSE<br>(Write in full)</th>
  <th colspan="2" width="11%">PERIOD OF ATTENDANCE</th>
  <th rowspan="2" width="11%">HIGHEST LEVEL/UNITS EARNED<br>(if not graduated)</th>
  <th rowspan="2" width="8%">YEAR GRADUATED</th>
  <th rowspan="2" width="13%">SCHOLARSHIP/ACADEMIC HONORS RECEIVED</th>
</tr>
<tr><th width="5.5%">From</th><th width="5.5%">To</th></tr>
${eduRows}
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>

<div class="sig-area">
  <div class="sig-box"><div style="font-size:7px;margin-bottom:2px">SIGNATURE</div><div class="sig-line"></div><div style="font-size:6.5px">(wet signature/e-signature/digital certificate)</div></div>
  <div class="sig-box" style="max-width:180px"><div style="font-size:7px;margin-bottom:2px">DATE</div><div class="sig-line"></div></div>
</div>
...
<table>
<tr>
  <th width="36%">27. CES/CSEE/CAREER SERVICE/RA 1080 (BOARD/BAR)/UNDER SPECIAL LAWS/CATEGORY II/IV ELIGIBILITY AND ELIGIBILITIES FOR UNIFORMED PERSONNEL</th>
  <th width="10%">RATING<br>(If Applicable)</th>
  <th width="13%">DATE OF EXAMINATION/<br>CONFERMENT</th>
  <th width="19%">PLACE OF EXAMINATION/<br>CONFERMENT</th>
  <th colspan="2" width="22%">LICENSE (if applicable)</th>
</tr>
<tr><td style="border:none;padding:1px"></td><td></td><td></td><td></td><th width="12%">NUMBER</th><th width="10%">Valid Until</th></tr>
${eligRows}
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>

<div class="shdr" style="margin-top:3px">V. WORK EXPERIENCE</div>
<div style="font-size:6.5px;margin-bottom:1px">(Include private employment. Start from your recent work.) Description of duties should be indicated in the attached Work Experience Sheet.</div>
<table>
<tr>
  <th colspan="2" width="19%">28. INCLUSIVE DATES<br>(dd/mm/yyyy)</th>
  <th width="25%">POSITION TITLE<br>(Write in full/Do not abbreviate)</th>
  <th width="28%">DEPARTMENT/AGENCY/OFFICE/COMPANY<br>(Write in full/Do not abbreviate)</th>
  <th width="10%">MONTHLY SALARY</th>
  <th width="11%">STATUS OF APPOINTMENT</th>
  <th width="7%">GOV'T SERVICE<br>(Y/N)</th>
</tr>
<tr><th width="9%">From</th><th width="10%">To</th><th></th><th></th><th></th><th></th><th></th></tr>
${weRows}
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>
...
<table>
<tr>
  <th width="42%">29. NAME &amp; ADDRESS OF ORGANIZATION<br>(Write in full)</th>
  <th colspan="2" width="22%">INCLUSIVE DATES (dd/mm/yyyy)</th>
  <th width="13%">NUMBER OF HOURS</th>
  <th width="23%">POSITION/NATURE OF WORK</th>
</tr>
<tr><th></th><th width="11%">From</th><th width="11%">To</th><th></th><th></th></tr>
${volRows}
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>
...
<table>
<tr>
  <th width="38%">30. TITLE OF LEARNING AND DEVELOPMENT INTERVENTIONS/TRAINING PROGRAMS<br>(Write in full)</th>
  <th colspan="2" width="22%">INCLUSIVE DATES OF ATTENDANCE (dd/mm/yyyy)</th>
  <th width="11%">NUMBER OF HOURS</th>
  <th width="13%">TYPE OF L&amp;D<br>(Managerial/Supervisory/Technical/etc)</th>
  <th width="22%">CONDUCTED/SPONSORED BY<br>(Write in full)</th>
</tr>
<tr><th></th><th width="11%">From</th><th width="11%">To</th><th></th><th></th><th></th></tr>
${trRows}
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>
...
<table>
<tr>
  <th width="33%">31. SPECIAL SKILLS and HOBBIES</th>
  <th width="34%">32. NON-ACADEMIC DISTINCTIONS / RECOGNITION<br>(Write in full)</th>
  <th width="33%">33. MEMBERSHIP IN ASSOCIATION/ORGANIZATION<br>(Write in full)</th>
</tr>
<tr>
  <td style="padding:4px;min-height:50px;vertical-align:top">${v(e.otherInfo.skills)}</td>
  <td style="padding:4px;vertical-align:top">${v(e.otherInfo.distinctions)}</td>
  <td style="padding:4px;vertical-align:top">${v(e.otherInfo.memberships)}</td>
</tr>
</table>
<div style="font-size:6.5px;margin-top:1px">(Continue on separate sheet if necessary)</div>
...
<td style="border:none;width:74%;vertical-align:top;padding-right:5px">

<table style="width:100%;margin-bottom:3px">
<tr><td colspan="3" style="padding:3px 4px;background:#f5f5f5"><span style="font-size:7.5px;font-weight:bold">34.</span>&nbsp;<span style="font-size:7.5px">Are you related by consanguinity or affinity to the appointing or recommending authority, or to the chief of bureau or office or to the person who has immediate supervision over you in the Office, Bureau or Department where you will be appointed,</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">a. within the third degree?</td><td style="padding:2px 6px;font-size:8px;text-align:center;width:30%">${yn(q.q34a)}</td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">b. within the fourth degree (for Local Government Unit - Career Employees)?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q34b)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q34det)}</span></td></tr>

<tr><td colspan="3" style="padding:2px 4px;background:#f5f5f5"><span style="font-size:7.5px;font-weight:bold">35.</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">a. Have you ever been found guilty of any administrative offense?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q35a)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q35aDet)}</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">b. Have you been criminally charged before any court?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q35b)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q35bDet)}</span></td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">Date Filed: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:100px">&nbsp;${v(q.q35bDate)}</span> &nbsp;&nbsp; Status of Case/s: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:90px">&nbsp;${v(q.q35bStatus)}</span></td></tr>

<tr><td colspan="2" style="padding:2px 4px;background:#f5f5f5;font-size:7.5px"><b>36.</b> Have you ever been convicted of any crime or violation of any law, decree, ordinance or regulation by any court or tribunal?</td><td style="padding:2px 6px;font-size:8px;text-align:center;background:#f5f5f5">${yn(q.q36)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q36Det)}</span></td></tr>

<tr><td colspan="2" style="padding:2px 4px;background:#f5f5f5;font-size:7.5px"><b>37.</b> Have you ever been separated from the service in any of the following modes: resignation, retirement, dropped from the rolls, dismissal, termination, end of term, finished contract or phased out (abolition) in the public or private sector?</td><td style="padding:2px 6px;font-size:8px;text-align:center;background:#f5f5f5">${yn(q.q37)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q37Det)}</span></td></tr>

<tr><td colspan="3" style="padding:2px 4px;background:#f5f5f5"><span style="font-size:7.5px;font-weight:bold">38.</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">a. Have you ever been a candidate in a national or local election held within the last year (except Barangay election)?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q38a)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q38aDet)}</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">b. Have you resigned from the government service during the three (3)-month period before the last election to promote/actively campaign for a national or local candidate?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q38b)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:220px">&nbsp;${v(q.q38bDet)}</span></td></tr>

<tr><td colspan="2" style="padding:2px 4px;background:#f5f5f5;font-size:7.5px"><b>39.</b> Have you acquired the status of an immigrant or permanent resident of another country?</td><td style="padding:2px 6px;font-size:8px;text-align:center;background:#f5f5f5">${yn(q.q39)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, give details (country): <span style="border-bottom:.5px solid #000;display:inline-block;min-width:160px">&nbsp;${v(q.q39Det)}</span></td></tr>

<tr><td colspan="3" style="padding:2px 4px;background:#f5f5f5;font-size:7.5px"><b>40.</b> Pursuant to: (a) Indigenous People's Act (RA 8371); (b) Magna Carta for Disabled Persons (RA 7277, as amended); and (c) Expanded Solo Parents Welfare Act (RA 11861), please answer the following items:</td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">a. Are you a member of any indigenous group?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q40a)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, please specify: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:160px">&nbsp;${v(q.q40aSpec)}</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">b. Are you a person with disability?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q40b)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 2px 18px;font-size:7px">If YES, please specify ID No: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:140px">&nbsp;${v(q.q40bId)}</span></td></tr>
<tr><td colspan="2" style="padding:2px 4px 2px 18px;font-size:7.5px">c. Are you a solo parent?</td><td style="padding:2px 6px;font-size:8px;text-align:center">${yn(q.q40c)}</td></tr>
<tr><td colspan="3" style="padding:1px 4px 3px 18px;font-size:7px">If YES, please specify ID No: <span style="border-bottom:.5px solid #000;display:inline-block;min-width:140px">&nbsp;${v(q.q40cId)}</span></td></tr>
</table>

<div style="font-size:7.5px;font-weight:bold;margin-bottom:2px">41. REFERENCES (Person not related by consanguinity or affinity to applicant/appointee)</div>
<table style="width:100%;margin-bottom:4px">
<tr><th width="38%">NAME</th><th width="36%">OFFICE/RESIDENTIAL ADDRESS</th><th width="26%">CONTACT NO. AND/OR EMAIL</th></tr>
${refs.map(r=>`<tr><td style="padding:2px 4px;min-height:12px">${v(r.name)}</td><td style="padding:2px 4px">${v(r.address)}</td><td style="padding:2px 4px">${v(r.contact)}</td></tr>`).join('')}
</table>

<div style="font-size:7.5px;margin-bottom:3px;line-height:1.5"><b>I declare under oath that I have personally accomplished this Personal Data Sheet which is a true, correct, and complete statement pursuant to the provisions of pertinent laws, rules, and regulations of the Republic of the Philippines.</b> I authorize the agency head/authorized representative to verify/validate the contents stated herein. I agree that any misrepresentation made in this document and its attachments shall cause the filing of administrative/criminal case/s against me.</div>

<table style="width:100%;margin-bottom:4px">
<tr>
  <td style="padding:2px 4px;width:70%"><span class="lbl">Government Issued ID (i.e. Passport, GSIS, SSS, PRC, Driver's License, etc.) — PLEASE INDICATE ID Number and Date of Issuance</span></td>
  <td rowspan="4" style="padding:4px;text-align:center;border:1px solid #888;width:30%">
    <div style="font-size:7px;margin-bottom:2px">Signature (Sign inside the box)</div>
    <div style="height:32px;border-bottom:.5px solid #000;width:90%;margin:0 auto"></div>
  </td>
</tr>
<tr><td style="padding:2px 4px"><span class="lbl">Government Issued ID:</span><span class="val">${v(e.govtId)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">ID/License/Passport No.:</span><span class="val">${v(e.govtIdNo)}</span></td></tr>
<tr><td style="padding:2px 4px"><span class="lbl">Date/Place of Issuance:</span><span class="val">${v(e.govtIdIssuance)}</span></td></tr>
</table>
<table style="width:100%;margin-bottom:3px">
<tr><td style="padding:2px 4px;width:50%"><span class="lbl">Date Accomplished:</span><span class="val">${v(e.dateAccomplished)}</span></td><td style="padding:2px 4px"><span class="lbl">Right Thumbmark</span><div style="height:35px;border:.5px solid #999;width:70px;margin-top:2px"></div></td></tr>
</table>

<div style="font-size:7.5px;margin-top:4px">SUBSCRIBED AND SWORN to before me this ___________________________, affiant exhibiting his/her validly issued government ID as indicated above.</div>
<div style="margin-top:18px;border-top:.5px solid #000;width:55%;font-size:7px;text-align:center;padding-top:2px">Person Administering Oath<br><span style="font-size:6.5px">(wet signature/e-signature/digital certificate except for notary public)</span></div>

</td>
<td style="border:1px solid #888;width:26%;vertical-align:top;padding:5px;text-align:center">
  <div style="font-size:7px;font-weight:bold;margin-bottom:3px">PHOTO</div>
  <div style="width:100%;height:110px;border:1px dashed #aaa;display:flex;align-items:center;justify-content:center;flex-direction:column;color:#aaa">
    <div style="font-size:8px">4 x 5 cm</div>
    <div style="font-size:7px;margin-top:2px">Photo Here</div>
  </div>
  <div style="margin-top:6px;font-size:6.5px;color:#555">(wet signature/e-signature/<br>digital certificate)</div>
  <div style="margin-top:14px;border-top:.5px solid #000;padding-top:2px;font-size:6.5px">Right Thumbmark</div>
</td>
</tr>
</table>
